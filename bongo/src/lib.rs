...
impl LayoutDisplay for ProductionElement {
    fn disp(&self) -> codefmt::Layout {
        match &self.identifier {
            Some(name) => {
                Layout::juxtapose(&[name.layout(), Layout::text(": "), self.element.disp()])
            }
...
impl Grammar {
    ...
    pub fn is_nullable(&self, nt: &NonTerminal) -> bool {
        ...
        if borrow.is_none() {
            let nullables =
                fixed_point(BTreeSet::new(), |nullables| is_nullable_fp(self, nullables));
...
fn is_prod_nullable(nullables: &BTreeSet<NonTerminal>, prod: &Production) -> bool {
    for elem in &prod.elements {
        match &elem.element {
            ...
            Element::NonTerm(nt) => if !nullables.contains(nt) {
                return false;
            },
        }
    }
...
mod tests {
    ...
    fn test_grammar_print() {
        ...
        let prod_set = ProductionSet::new(vec![Production::new("Recursive", v1), Production::new("Empty", v2)]);
